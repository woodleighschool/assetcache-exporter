...
- Apple Content Caching status and metrics are independent inputs. Preserve useful metrics when either source is unavailable.
- Metric names, labels, and meanings are public scrape contracts. Interval rows from `Metrics.db` represent gauges.
- Tests use injected sources and fixtures. Packaging and LaunchDaemon behaviour live under `packaging/` and follow the existing Go-to-sign-to-package release flow.
